...
class MessageSync {
   public:
    using Callback = std::function<void(const MeasureGroup &)>;//返回值（输入值）

    MessageSync(Callback cb) : callback_(cb), conv_(new CloudConvert) {}  //定义回调函数
    ...
    void ProcessIMU(IMUPtr imu) {
        ...
        if (timestamp < last_timestamp_imu_) {  //时间戳要大于上一个IMU时间戳
            LOG(WARNING) << "imu loop back, clear buffer";
            imu_buffer_.clear();
        }

        last_timestamp_imu_ = timestamp;
        imu_buffer_.emplace_back(imu); ///读入IMU
    }

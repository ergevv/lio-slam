 ...
 * .eval() 方法用于强制执行延迟计算。在 Eigen 中，很多操作都是懒惰计算（lazy evaluation）的，这意味着实际的计算可能会延迟到真正需要结果的时候才进行。这种机制有助于优化性能，因为它可以合并多个操作，从而减少不必要的内存拷贝和计算步骤。然而，在某些情况下，你可能希望立即执行计算并获取结果，这时就可以使用 .eval() 方法。
 */
template <typename C, typename D, typename Getter>
void ComputeMeanAndCovDiag(const C& data, D& mean, D& cov_diag, Getter&& getter) {
    size_t len = data.size();
    assert(len > 1);  //false则报错
    // clang-format off
...
inline bool PoseInterp(double query_time, C&& data, FT&& take_time_func, FP&& take_pose_func, SE3& result,
    ...
    double last_time = take_time_func(*data.rbegin()); //逆序迭代器,最后一个元素
    ...
    for (auto iter = data.begin(); iter != data.end(); ++iter) {  //imu状态
        auto next_iter = iter;
        next_iter++;

        if (take_time_func(*iter) < query_time && take_time_func(*next_iter) >= query_time) {  //scan在哪个区间
            match_iter = iter;
            break;
        }
    }
    ...
    double dt = take_time_func(*match_iter_n) - take_time_func(*match_iter); //sacn的前后imu
    ...
    result = {pose_first.unit_quaternion().slerp(s, pose_next.unit_quaternion()),  //插值
              pose_first.translation() * (1 - s) + pose_next.translation() * s};
    best_match = s < 0.5 ? *match_iter : *match_iter_n; //与哪一个更近
    return true;
}

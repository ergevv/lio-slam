...
void IMUPreintegration::Integrate(const IMU &imu, double dt) {
    // 去掉零偏的测量
    Vec3d gyr = imu.gyro_ - bg_;  // 陀螺，为什么用0的，不是初始化已经算出来了吗
    Vec3d acc = imu.acce_ - ba_;  // 加计

    // 预积分观测量更新dv, dp, 见(4.13), (4.16)
    dp_ = dp_ + dv_ * dt + 0.5f * dR_.matrix() * acc * dt * dt;  //
    dv_ = dv_ + dR_ * acc * dt;

    // dR先不更新，因为A, B阵还需要现在的dR，噪声的传递，用作信息矩阵
    ...
    Mat3d acc_hat = SO3::hat(acc); //向量-》反对称矩阵
    double dt2 = dt * dt;
...
NavStated IMUPreintegration::Predict(const sad::NavStated &start, const Vec3d &grav) const {
    SO3 Rj = start.R_ * dR_;    //4.18？ 忽略噪声，并把最终值移到左边，噪声符合零均值的高斯分布
